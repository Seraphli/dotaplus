...
class CountersSpider(scrapy.Spider):
    ...
    def parse(self, response):
        ...
        hero_name = []
        for i in [2, 4, 6]:
            hero_href.extend(response.css('#mw-content-text > div '
                                          'table:nth-child(2) > tbody '
                                          f'tr:nth-child({i}) > td > div > '
                                          'div > a::attr(href)').extract())
            hero_name.extend(response.css('#mw-content-text > div '
                                          'table:nth-child(2) > tbody '
                                          f'tr:nth-child({i}) > td > div > '
    ...
    def parse_counters(self, response):
        ...
        all_hero = response.css('#mw-content-text > div > div > b > '
                                'a::attr(title)').extract()
        split_hero = response.css('#mw-content-text > div > h2+div+div > b > '
                                  'a::attr(title)').extract()
