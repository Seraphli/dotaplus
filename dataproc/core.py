...
class Core(object):
    ...
    def load_data(self, path):
        ...
        self.num_heroes = len(self.web_data.keys())
        self.weight = np.arange(self.num_heroes, 0, -1)
    ...
    def get_reason(self, hero, reasons, reason, check):
        from dataproc.reason import CN_REASON_DICT
        _reason = []
    ...
    def calculate_based_on_cfg(self, config_str):
        ...
        for h in self.web_data:
            total_pos = 0
            total_neg = 0
            for other in self.web_data[h]['c_match_ups_' + config_str]:
                index = self.web_data[h]['c_match_ups_' + config_str][other]
                if index > 0:
                    total_pos += index
                else:
                    total_neg += index
        ...
        for h in self.web_data:
            total_pos = 0
            total_neg = 0
            for other in self.web_data[h]['c_teammates_' + config_str]:
                index = self.web_data[h]['c_teammates_' + config_str][other]
                if index > 0:
                    total_pos += index
                else:
                    total_neg += index
    ...
    def cal_match(self, config_str, hero_value, reasons,
        ...
        for hero in match_ups:
            sorted_list = []
            for other in self.web_data[hero]['c_match_ups_' + config_str]:
                sorted_list.append([other, -self.web_data[hero]
                ['c_match_ups_' + config_str][other]])
            sorted_list = sorted(sorted_list, key=lambda x: x[1])
        ...
        for hero in teammates:
            sorted_list = []
            for other in self.web_data[hero]['c_teammates_' + config_str]:
                sorted_list.append([other, self.web_data[hero]
                ['c_teammates_' + config_str][other]])
            sorted_list = sorted(sorted_list, key=lambda x: x[1])
...
if __name__ == '__main__':
    ...
    r = core.calculate(cfg, ['witch_doctor', 'skeleton_king', 'juggernaut',
                             'drow_ranger', 'omniknight'],
                       ['monkey_king', 'nevermore', 'windrunner', 'kunkka',
                        'zuus'], CUSTOM_DATA['cn_layout'])
    print(r)
